...
const Navbar = () => {
  ...
  return (
    ...
    px-4 sm:px-20 xl:px-32'>
        <img src={assets.logo} alt='logo' className='w-20 h-20 sm:w-44 cursor-pointer' onClick={()=>navigate('/')}/>
         
        
        {
            user ? <UserButton/>:(
                <button onClick={openSignIn} className='flex items-center gap-2 rounded-full text-sm 
                cursor-pointer bg-green-600 text-white px-10 py-2.5'>Get started <ArrowRight className='w-4 h-4'/>
                </button>
            )
        }
